...
- **Motor nativo** (`native/`, `docs/ENGINE.md`, `engine.test.ts`) — el núcleo en C++ para llegar al millón de neuronas: neuronas de Izhikevich en arrays con **paridad** probada contra el modelo TypeScript, sinapsis dispersas, **interneuronas inhibitorias reales** en lugar de la competición algorítmica, STDP por trazas, hilos; kernels CUDA escritos a la espera de una GPU. En un M3 Max, **un millón de neuronas** con cien sinapsis cada una corre a unos 34 ticks por segundo, más de tres veces el tiempo real a 10 Hz. La primera región sobre él, la **corteza nativa** (`GBRAIN_NATIVE=1`), forma asambleas reproducibles y dispersas sin competición algorítmica y **completa el patrón**: con aferentes topográficas, corrientes sinápticas con constante de tiempo, depresión a corto plazo y plasticidad estructural e inhibitoria, media entrada trae de vuelta el 85–100 % del lado de la asamblea que la pista no alcanza, solo para su propia entrada, y la asamblea se apaga sola. La conectividad se construye por **bloques** (recurrentes locales y de largo alcance, proyecciones entre áreas): dos áreas en un mismo motor se relevan, aprenden y completan a través de la proyección, pero la segunda aún no distingue entradas (medido y declarado; lo siguiente son la normalización sináptica y las corrientes NMDA).
...
- **Native engine** (`native/`, `docs/ENGINE.md`, `engine.test.ts`) — the C++ core on the way to a million neurons: Izhikevich neurons in arrays with **parity** proven against the TypeScript model, sparse synapses, **real inhibitory interneurons** instead of algorithmic competition, trace STDP, threads; CUDA kernels written, waiting for a GPU. On an M3 Max, **a million neurons** with a hundred synapses each run at about 34 ticks per second, over three times real time at 10 Hz. The first region on it, the **native cortex** (`GBRAIN_NATIVE=1`), forms reproducible, sparse assemblies with no algorithmic competition and **completes the pattern**: with topographic afferents, synaptic currents with a time constant, short-term depression and structural and inhibitory plasticity, half the input brings back 85–100% of the side of the assembly the cue does not reach, only for its own input, and the assembly fades on its own. Connectivity is built from **blocks** (local and long-range recurrents, projections between areas): two areas on one engine relay, learn and complete across the projection, but the second area does not yet tell inputs apart (measured and stated; synaptic normalization and NMDA currents are next).
